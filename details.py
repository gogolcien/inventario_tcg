...
def new_context(browser):
    ...
    def _block_heavy_resources(route):
        req = route.request
        # Dejamos pasar las imagenes del CDN de producto (necesitamos que
        # carguen para que el componente "v-lazy-image" reemplace el
        # placeholder base64 por la URL real). Bloqueamos el resto de
        # imagenes/fuentes/CSS/video para mantener la velocidad.
        if req.resource_type == "image" and "tcgplayer-cdn.tcgplayer.com" in req.url:
            route.continue_()
        elif req.resource_type in ("image", "media", "font", "stylesheet"):
            route.abort()
        else:
            route.continue_()
...
def get_image_url(page, timeout=8000):
    """
    ...
    El componente "v-lazy-image" arranca con un placeholder base64 en 'src'
    y recien lo reemplaza por la URL real una vez que la imagen termina de
    cargar en el navegador -- por eso esperamos explicitamente a que eso
    pase antes de leer el atributo. Devuelve la version mas grande
    disponible (usa el srcset si esta presente).
    """
    try:
        img = page.locator('img[data-testid^="product-image__container"]').first
        img.scroll_into_view_if_needed(timeout=timeout)
        img.wait_for(state="attached", timeout=timeout)

        # Esperar a que el src deje de ser el placeholder "data:image/gif;base64,..."
        try:
            page.wait_for_function(
                "(el) => el.getAttribute('src') && !el.getAttribute('src').startsWith('data:')",
                arg=img.element_handle(),
                timeout=timeout,
            )
        except PWTimeout:
            pass  # seguimos igual con lo que haya, mejor que nada
